     ...
     1        nFlt0, nFtype(MAX_FLT), faultflag(MAX_FLT,MAX_SEGMOD,MAX_SEG),
     2        f_start(MAX_FLT), f_num(MAX_FLT), nSegModel(MAX_FLT), nFltTotal
      real al_segWt(MAX_FLT), cumWt_segModel(MAX_FLT,MAX_SEGMOD), version,
     1     cumWt_param(MAX_FLT,MAX_WIDTH,MAXPARAM), ProbAct(MAX_FLT),
     2     cumWt_width(MAX_FLT,MAX_WIDTH), cumwt_Ftype(MAX_FLT,MAXPARAM)
...
c ----------------------------------------------------------------------
     ...
     2           nFtype, nWidth, nSegModel, f_start, f_num, faultFlag, al_Segwt )
      ...
      integer nWidth(MAX_FLT), nParamVar(MAX_FLT,MAX_WIDTH), Attentype(MAX_FLT),
     1        nFlt0, nFlt2, nFm, nFtypeModels(MAX_FLT), nFtype1(MAXPARAM),
     ...
     4        nFtype(MAX_FLT), faultflag(MAX_FLT,MAX_SEGMOD,MAX_SEG)
      integer f_start(MAX_FLT), f_num(MAX_FLT), nSegModel(MAX_FLT), iWidth, iDip,
     ...
     4        iDepthModel, nfp, nsyn, nFltTotal, iRefMag
      real segwt(MAX_FLT,MAX_FLT), wt, al_segWt(MAX_FLT), minmag, magstep,
     1     magRecurWt(MAXPARAM), dipWt(MAX_WIDTH), bValueWt(MAXPARAM), minDepth,
     2     cumWt_segModel(MAX_FLT,MAX_SEGMOD), cumWt_param(MAX_FLT,MAX_WIDTH,MAXPARAM),
     3     cumWt_width(MAX_FLT,MAX_WIDTH), cumwt_Ftype(MAX_FLT,MAXPARAM),
     4     x(MAXPARAM), ProbAct(MAX_FLT)
      real ftmodelwt(MAXPARAM), ftype1(MAXPARAM,MAXPARAM), Ftype_wt1(MAXPARAM,MAXPARAM),
     1     rateType1(MAXPARAM), width_wt(MAX_WIDTH), magsyn, rupsyn, jbsyn,
...
c       Read number of segmentation models for this fault system
        ...
        call CheckDim ( nSegModel(iFlt0), MAX_SEGMOD, 'MAX_SEGMOD   ' )
...
c       Read total number of fault segments for this fault system
        read (10,*) nFlt2
        call CheckDim ( nFlt2, MAX_SEG, 'MAX_SEG   ' )
...
c         Read dip Variation
          if ( isourceType .ne. 5 ) then
            read (10,*) n_Dip(iflt)
            call CheckDim ( n_Dip(iflt), MAX_WIDTH, 'MAX_WIDTH ' )
...
c         Read b-values (not for activity rate cases)
          read (10,*) n_bValue
          call CheckDim ( n_bValue, MAXPARAM, 'MAXPARAM    ' )
...
c         Read activity rate - b-value pairs
          read (10,*) nActRate
          call CheckDim ( nActRate, MAXPARAM, 'MAXPARAM    ' )
...
c         Read slip-rates
          read (10,*) nSR
          call CheckDim ( nSR, MAXPARAM, 'MAXPARAM    ' )
          if ( nSR .gt. 0 ) then
            read (10,*) (x(k),k=1,nSR)
...
c         Read recurrence intervals
          read (10,*) nRecInt
          call CheckDim ( nRecInt, MAXPARAM, 'MAXPARAM    ' )
...
c         Read moment-rates
          read (10,*) nMoRate
          call CheckDim ( nMoRate, MAXPARAM, 'MAXPARAM    ' )
...
c         Load into single array called "rate_param"
          ...
          call CheckDim ( nRate, MAXPARAM, 'MAXPARAM    ' )
          do k=1,nSR
...
c         Read Mag recurrence weights (char and exp)
          read (10,*) nMagRecur
          call CheckDim ( nMagRecur, MAXPARAM, 'MAXPARAM    ' )
...
c         Read reference mags for each fault thickness
          ...
          do iThick1=1,nThick1(iFlt)
            ...
            call CheckDim ( nRefMag(iThick), MAXPARAM, 'MAXPARAM    ' )
...
c         Read ftype Models
          ...
          call CheckDim ( nFtypeModels(iFlt), MAXPARAM, 'MAXPARAM    ' )
          do iFM=1,nFtypeModels(iFlt)
            read (10,*) ftmodelwt(iFM)
            read (10,*) nFtype1(iFM)
            call CheckDim ( nFtype1(iFM), MAXPARAM, 'MAXPARAM    ' )
...
c        Load up Ftype Models and weights.
         ...
         enddo
         nFm = nFm - 1
         call CheckDim ( nFm, MAXPARAM, 'MAXPARAM    ' )
         nFtype(iFlt) = nFm
...
c     Load up parameter variations into large single dimension arrays
         ...
         do iThick=1,nThick1(iFlt)
           do iDip=1,n_Dip(iFlt)
             i1 = i1 + 1
             call CheckDim ( i1, MAX_WIDTH, 'MAX_WIDTH' )
...
c ----------------------------------------------------------------------
     ...
     3        nSR, nActRAte, nRecInt, n_Dip(MAX_FLT), nRefMag(MAX_WIDTH),
     4        faultflag(MAX_FLT,MAX_SEGMOD,MAX_SEG), nsyn, nFltTotal
      integer f_start(MAX_FLT), f_num(MAX_FLT), nSegModel(MAX_FLT), iWidth, iDip,
     1        iRefMag, iDepthModel, nfp, ii, ipt, nDownDip, insyn, iRecur,
     2        iflt, iflt0, iflt2, k, i, iCoor, isourceType, i_bValue, nb1,
     3        iRate, i1, iFM, nRupArea, nRupWidth, iThick, iThick1, nMoRate,
     4        nRate, nMagRecur, n_bValue
      real segwt(MAX_FLT,MAX_FLT), wt, al_segWt(MAX_FLT), minmag, magstep,
     1     magRecurWt(MAXPARAM), dipWt(MAX_WIDTH), bValueWt(MAXPARAM), minDepth,
     2     cumWt_segModel(MAX_FLT,MAX_SEGMOD), cumWt_param(MAX_FLT,MAX_WIDTH,MAXPARAM),
     3     cumWt_width(MAX_FLT,MAX_WIDTH), cumwt_Ftype(MAX_FLT,MAXPARAM),
